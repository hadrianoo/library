...
class DOM {
  ...
  appendRow(book, index) {
    const { tr, itemNum, title, author, pages, userRead, remove } =
      this.createTableRow();
    ...
    userRead.appendChild(
      this.createButton("toggle", book.read ? "Yes" : "No", book.id),
    );
    tr.appendChild(userRead);
    remove.appendChild(
      this.createButton("remove-button", "Remove book", book.id),
    );
